...
- **v1.1.2 Release - Super Robot Wars Heavy Beam, Rotatable 3-Hex Orbiting Array Weapon & Accessory Return Solver Overhaul:**
  ...
  - **Accessory Return & AutoEquip Solver Overhaul**: Movable `Accessory Return` tiles on the Torso are now recognized by the `AutoEquipSolver` as inbound power injectors, cleanly routing returned energy from Head and Backpack into the Torso's main grid.
