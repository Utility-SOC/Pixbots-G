...
- **Test Range: HP override field (captured 2026-07-22):** a text box in the Garage Test Range letting the player type in a custom HP value for the target dummy, instead of whatever the dummy's default/current HP is - lets a player check "how many hits of my current build to kill something with exactly N HP" (e.g. matching a specific boss's known HP) without needing to actually fight it first. Small, self-contained UI addition to `GarageTestRange.gd`'s existing dummy setup.
...
- **More "transparent" hex tiles (captured 2026-07-22):** the tile roster is short on simple, low-commitment structural options - Directional Conduit is the only real "does basically nothing, just routes energy" tile, Amplifier is rare, and a player who doesn't want to commit a cell to Infusion or Catalyst's bigger effects has few other low-key choices. Add more tile types that are intentionally unexciting/passthrough-ish, giving builds more flexible filler between the high-commitment tiles instead of forcing every open cell toward one of a small set of "big" options.
