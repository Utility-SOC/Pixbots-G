...
var power_infusion: float = TileStatsRegistry.get_stat("InfuserTile", "power_infusion", 2.0)

func _init():
	super._init("Elemental Infuser", HexTile.TileCategory.PROCESSOR)

func get_weight() -> float:
	return TileStatsRegistry.get_stat("InfuserTile", "weight", 4.0) # moderate - an elemental conversion processor
...
func process_energy(packet: EnergyPacket, entry_direction: int, grid: Node = null, entry_coord: HexCoord = null) -> Array[EnergyPacket]:
	...
	var conversion_rate = TileStatsRegistry.get_stat("InfuserTile", "conversion_rate_base", 0.4) \
		+ (rarity * TileStatsRegistry.get_stat("InfuserTile", "conversion_rate_rarity_coeff", 0.15)) \
		+ ((level - 1) * TileStatsRegistry.get_stat("InfuserTile", "conversion_rate_level_coeff", 0.05))
	...
	var infusion_amount = power_infusion * (1.0 + rarity * TileStatsRegistry.get_stat("InfuserTile", "infusion_rarity_coeff", 0.5)) * (1.0 + (level - 1) * TileStatsRegistry.get_stat("InfuserTile", "infusion_level_coeff", 0.1))
